...
seed_i <- as.integer(snakemake@input[['seed']])
set.seed(seed_i)
...
dds_metadata$toxicity <- factor(sample(dds_metadata$toxicity, length(dds_metadata$toxicity), replace=F))
...
write_tsv(results(deseq_full), paste0("results/tox_de.lrt.donor_celltype_intx.", seed_i, ".tsv")
